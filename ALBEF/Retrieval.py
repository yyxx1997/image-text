...
from models.model_te import BertTEModel, BertTEEvalDataset
...
def evaluation_only_i2t(model, data_loader, tokenizer, device, config):
    ...
    image_embeds = torch.cat(image_embeds,dim=0)
    sims_matrix = image_embeds @ text_embeds.t()
    score_matrix_i2t = torch.full((image_embeds.shape[0],len(texts)),-100.0).to(device)
    ...
    for image_id,txt_ids in result.items():
        topk_result[data_loader.dataset.image[image_id]]={"goldens":[texts[txtid] for txtid in img2txt[image_id]],"topks":[texts[txtid] for txtid in txt_ids]}
    with open('/data1/yx/suda/image-text/sotas/ALBEF/output/topk_result_{}.json'.format(len(image_ids)),'w',encoding="utf8") as file:
        file.write(json.dumps(topk_result,ensure_ascii=False,indent=4))
    pairs=[]
    for img_id,contents in result.items():
        goldens=img2txt[img_id]
        for txt_id in goldens:
            if txt_id >= texts.__len__():
                continue
            pairs.extend([(txt_id,content) for content in contents if txt_id != content and content not in goldens])

    TE_eval_dataset=BertTEEvalDataset(pairs,image_ids,texts)
    dev_dataloader = DataLoader(TE_eval_dataset,batch_size=64)
    entailments={}
    for i,(premise,hyposis,index) in tqdm(enumerate(dev_dataloader),desc="Get Entailments..."): 
        inputs = tokenizer(premise,hyposis,
                padding=True,
                truncation=True,
                return_tensors="pt",
                max_length=512)
        ...
        bert_output = model.te(input_ids, input_tyi, input_attn_mask)
        res=bert_output.argmax(dim=1).tolist()
        for idx,i in zip(index,res):
            if i==0:
                origin_pre_id,origin_hypo_id=pairs[idx]
                if origin_pre_id not in entailments.keys():
                    entailments[origin_pre_id]=set()
                entailments[origin_pre_id].add(origin_hypo_id)
    ...
    for img_id,contents in result.items():
        ...
        unions=set()
        for txt_id in goldens:
            if txt_id not in entailments.keys():
                continue
            unions.update(entailments[txt_id])
        entails+=len(unions)
    
    entails_recover=entails/(check_entail*len(image_ids))
    print("top {} 中的蕴含比例为：{}".format(check_entail,entails_recover))
    ...
    pr1= 100.0 * np.sum(pres[:,0]) / (pres.shape[0]*1)
    pr5= 100.0 * np.sum(pres[:,4]) / (pres.shape[0]*5)
    pr10= 100.0 * np.sum(pres[:,9]) / (pres.shape[0]*5)
    
    tr1 = 100.0 * len(np.where(ranks < 1)[0]) / len(ranks)
    tr5 = 100.0 * len(np.where(ranks < 5)[0]) / len(ranks)
    tr10 = 100.0 * len(np.where(ranks < 10)[0]) / len(ranks)
    tr_mean = (tr1 + tr5 + tr10) / 3
    pr_mean = (pr1+pr5+pr10)/3
...
def main(args, config):
    ...
    train_dataset, extra_dataset, val_dataset, test_dataset = create_dataset('re', config)  
    ...
    train_loader, extra_loader, val_loader, test_loader = create_loader([train_dataset, extra_dataset, val_dataset, test_dataset],samplers,
    ...
    te_checkpoint = torch.load(config['te_path'],map_location='cpu')
    te = BertTEModel(2)
    te.load_state_dict(te_checkpoint['model'])
    model.te = te
    ...
    for epoch in range(0, max_epoch):
        ...
        i2tres=evaluation_only_i2t(model_without_ddp,val_loader, tokenizer, device, config)
        print(i2tres)
